...
function holdDown(tokenElement, event) {
  ...
  if (moving == false) {
    ...
    movingElement.style.left =
      event.clientX - movingElement.clientWidth / 2 + "px";
    movingElement.style.top =
      event.clientY - movingElement.clientHeight / 2 + "px";
      ...
      if (moving) {
        ...
        if (activeElement && activeElement.userToken) {
          ...
          activeElement.style.left =
            event.clientX - activeElement.clientWidth / 2 + "px";
          activeElement.style.top =
            event.clientY - activeElement.clientHeight / 2 + "px";

          // calculate Y delta
          let deltaY =
            event.clientY - (rect.y + activeElement.clientHeight / 2);
          console.log(deltaY);
