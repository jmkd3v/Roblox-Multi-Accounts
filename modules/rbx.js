...
export function generateEndpoint(api, version, endpointPath) {
	return "https://" + api + "." + baseSite + "/v" + version + "/" + endpointPath
}
...
export async function getAuthenticatedUser() {
	let response = await fetch(generateEndpoint("users", "1", "users/authenticated"));
	if (response.ok) {
...
export async function getUser(userId) {
	let response = await fetch(generateEndpoint("users", "1", "users/" + userId.toString()))
	if (response.ok) {
		return response.json();
	} else {
		return false;
